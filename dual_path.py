...
class Dual_Path_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        for x_layer in x_layers:
            x_after_layer.append( self.x_after( x_layer, gap ) )
        ...
        return x_after_layer

    def x_after(self, x, gap ):
...
class SepformerWrapper(nn.Module):
    ...
    def forward(self, mix):
        ...
        est_mask_layers = self.masknet(mix_w)
        mix_w = torch.stack([mix_w] * self.num_spks)
        #sep_h = mix_w * est_mask
        sep_h_layers = []
        for est_mask_layer in est_mask_layers:
            sep_h_layers.append( mix_w * est_mask_layer )

        #sep_h_layers = torch.stack( sep_h_layers )
        ...
        #)
        est_source_layers = []
        for sep_h_layer in sep_h_layers:
            est_source_layers.append( 
                torch.cat( 
                    [ 
                        self.decoder(sep_h_layer[i]).unsqueeze(-1)
                        for i in range( self.num_spks) 
                    ], 
                dim = -1, 
                )
        ...
        est_source_layers = torch.stack( est_source_layers )

        # T changed after conv1d in encoder, fix it here
        T_origin = mix.size(1)
        T_est = est_source_layers.size(2)
        est_source_layers2 = []
        if T_origin > T_est:
            #est_source = F.pad(est_source, (0, 0, 0, T_origin - T_est))
            for est_source_layer in est_source_layers:
                est_source_layers2.append( F.pad(est_source_layer, (0, 0, 0, T_origin - T_est)) )
        else:
            #est_source = est_source[:, :T_origin, :]
            for est_source_layer in est_source_layers:
                est_source_layers2.append( est_source_layer[:, :T_origin, :] )


        est_source_layers2 = torch.stack( est_source_layers2 )

        return est_source_layers2
        #print( "1 est_source size:", est_source.size() )
        #print( " permutes est_size:", est_source.permute( 1, 0, 2 ).size() ) 
        #return est_source.permute( 1, 0, 2 )
        #return est_source.transpose( 1, 2 )
        #return est_source.permute( 2, 0, 1 )
